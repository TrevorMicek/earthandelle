...
const navigation = {
  ...
  export default function Footer() {
    return (
      <footer className="bg-light" style={{gridColumn: 'span 5'}}>
          ...
          <nav className="-mx-5 my-2 flex flex-col flex-wrap items-center justify-center" aria-label="Footer">
            {navigation.main.map((item) => (
              <div key={item.name} className="px-2 py-2">
                <a href={item.href} className="text-base text-default hover:text-gray-900">
                  {item.name}
                </a>
              </div>
            ))}
          </nav>
          <div className="bg-accent w-full py-3">
            <img src={logo} width="200px" height="150px" className=" pt-3 mx-auto" />
          <div className="mt-8 flex justify-center space-x-6">
            {navigation.social.map((item) => (
              <a key={item.name} href={item.href} className="text-default hover:text-gray-500">
                <span className="sr-only">{item.name}</span>
                <item.icon className="h-6 w-6" aria-hidden="true" />
              </a>
            ))}
          </div>
          <p className="mt-6 text-center text-sm text-default">&copy; 2022, earthandelle</p>
        </div>
        </div>
      </footer>
    )
