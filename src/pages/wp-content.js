...
const IndexPage = () => {
  ...
  //wp-content/uploads/2021/10/Vegan-Collagen-Body-Boost.pdf.pdf.pdf?
  const BlogPost = () => {
    const id = useMatch("/wp-content/:blogId").blogId
    console.log(id)
    useEffect(() => {
      setPost(id)
      window.scrollTo(0,0)
  ...
  <Layout title="Blog" text="Learn About Web Design, SEO, Ecommerce & More!">
    <SEO title="Blog" />
      ...
      <Router className=" relative bg-white pt-4 -mb-8 overflow-hidden sm:pt-24 lg:pt-32" style={{gridColumn:"span 7", gridRowStart:"first", gridRowEnd:"second"}}>
        <BlogSection path="/wp-content" post={posts} />
        <BlogPost path="/wp-content/:blogId" posts={posts}>blog article</BlogPost>
      </Router>
  </Layout>
  )
  }
